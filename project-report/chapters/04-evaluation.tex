...
validation \texttt{mAP@50:95} (epoch 40 generic, epoch 143 ball).

\subsection{Generic four-class detector}

The generic detector achieves a mean test \texttt{mAP@50} of $0.855$
...
($\text{mAP@50} \geq 0.98$, $\text{F1} \geq 0.96$), whereas the Ball
class reaches only $\text{mAP@50} = 0.459$ at recall $0.39$. The
...
class: test \texttt{mAP@50} rises from $0.459$ to $0.950$
($+49.1$~pp), and operational recall (IoU=$0.45$, conf=$0.25$) from
...
\paragraph{Ranking with similar work.}
The generic detector's $0.855$ \texttt{mAP@50} sits within the range
...
\paragraph{Verdict on success criterion (i).}
\textbf{Met.} Generic-detector \texttt{mAP@50} = $0.855 \geq 0.80$.
...
ByteTrack achieves higher HOTA ($66.8$ vs $65.7$) and IDF1 ($67.8$ vs
$67.1$) and records approximately $27\%$ fewer identity switches
...
the gap on these sequences. The combined-set HOTA of $66.8$ is close
to the SportsMOT results of Cui~et~al.~\cite{Cui2023SportsMOT}:
ByteTrack reports $62.8$--$64.1$ on the overall test set (Train vs
Train+Val protocol), and MixSort-Byte reports $66.4$ on the football
...
\paragraph{Verdict on success criterion (ii).}
\textbf{Met.} Deployed-tracker HOTA is $66.8$ against a target of
...
The SigLLIP-based classifier's $0.99{+}$ figures are consistent with the expected gain from
...
\paragraph{Keypoint detector.}
The YOLOv8x-pose model achieves test pose \texttt{mAP@50} = $0.995$
and \texttt{mAP@50:95} = $0.953$ on the 28-image held-out split, with
...
\midrule
i   & Detector mAP@50 & $\geq 0.80$ & $0.855$ generic; $0.950$ ball
    & \S\ref{sec:eval_detection} \\
ii  & Tracking HOTA & $\geq 0.45$ & $66.8$ (ByteTrack)
