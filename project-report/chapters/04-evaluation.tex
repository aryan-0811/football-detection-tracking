...
% This chapter evaluates the pipeline against the six success criteria
% committed in Chapter~\ref{ch:introduction}: detector $\mathrm{mAP}_{50}
% \geq 0.80$, tracking $\mathrm{HOTA} \geq 0.45$, team-assignment
% macro-$F_1 \geq 0.85$, median pitch-space reprojection error
% $\leq 250$\,cm, pass-detection $F_1 \geq 0.80$, and qualitatively
% coherent bird's-eye, heatmap, and offside outputs. Each criterion is
% addressed by the section that owns the corresponding pipeline stage,
% with verdicts issued explicitly where the available evidence permits one.
...
$\leq 250$\,cm, pass-detection $F_1 \geq 0.80$, and qualitatively
coherent bird's-eye, heatmap, and offside outputs. Each criterion is
addressed in the section covering the relevant pipeline stage, with
explicit verdicts where the evidence permits.

Evaluation operates at three levels of evidence.
\emph{Component-level} evaluation
(Sections~\ref{sec:eval_detection} and~\ref{sec:eval_pitch}) uses the
held-out test splits of the training datasets and reports the standard
object-detection and pose metrics defined in
Sections~\ref{subsec:background_iou_nms}
and~\ref{subsec:background_tracking_metrics}, allowing direct
comparison with the broadcast-football literature
\cite{Cioppa2022,Theiner2022,Magera2025}.
\emph{Integration-level} evaluation
(Sections~\ref{sec:eval_tracking} and~\ref{sec:eval_team})
characterises stages whose behaviour depends on temporal context —
tracking continuity and team-label stability — using a combined
SportsMOT-football~\cite{Cui2023SportsMOT} and broadcast tracking set,
and three independent clips with manually labelled torso crops for
team assignment.
\emph{System-level} evaluation
(Sections~\ref{sec:eval_tactical} and~\ref{sec:eval_runtime}) examines
end-to-end outputs — event logs, per-player statistics, heatmaps,
runtime — on full pipeline runs against the same three clips, with
event ground truth obtained by visual inspection of the annotated
outputs.

The evaluation is scoped by the failure-handling table of
Section~\ref{sec:methodology_failure_handling}: claims are made only
within its operational regime, and degraded-input behaviour is
characterised in Section~\ref{sec:eval_failures} rather than treated
as out-of-scope. Omitted metrics or comparisons are flagged and
...
The two detectors are evaluated on held-out test splits (75 images /
1{,}875 instances for the generic detector; 119 images / 119 instances
for the ball detector) using the COCO-style protocol of
Section~\ref{subsec:background_iou_nms}. Weights were selected by
validation \texttt{$\mathrm{mAP}_{50-95}$} (epoch 40 generic, epoch 143 ball).

\subsection{Generic four-class detector}

The generic detector achieves a mean test \texttt{$\mathrm{mAP}_{50}$} of $0.855$
(Table~\ref{tab:detector_per_class}), exceeding the criterion~(i)
target of $0.80$. The aggregate masks a sharply bimodal per-class
distribution: Player, Goalkeeper, and Referee are near-saturated
($\text{$\mathrm{mAP}_{50}$} \geq 0.98$, $\text{F1} \geq 0.96$), whereas the Ball
class reaches only $\text{$\mathrm{mAP}_{50}$} = 0.459$ at recall $0.39$. The
precision--recall curves (Figure~\ref{fig:generic_pr}) confirm this:
the person classes track the upper envelope while the Ball curve
collapses below recall $0.8$, isolating small-object localisation as
...
\begin{tabular}{lrrrrr}
\toprule
Class & P & R & F1 & $\mathrm{mAP}_{50}$ &$\mathrm{mAP}_{50-95}$ \\
\midrule
Ball       & 0.818 & 0.393 & 0.531 & 0.459 & 0.194 \\
Goalkeeper & 0.953 & 0.983 & 0.968 & 0.982 & 0.808 \\
Player     & 0.991 & 0.991 & 0.991 & 0.995 & 0.823 \\
...
envelope of the plot, while the Ball class collapses sharply once
recall exceeds $0.75$, accounting for the bulk of the gap between
mean \texttt{$\mathrm{mAP}_{50}$} and the person-class scores.}
\label{fig:generic_pr}
\end{figure}

The normalised confusion matrix (Figure~\ref{fig:generic_cm_norm})
localises two failure modes. First, $31\%$ of true balls are confused
with background and $40\%$ of background predictions are labelled
ball: small target size and low contrast against pitch grain dominate
ball detection at $1280\times1280$, consistent with~\cite{Cioppa2022}.
Second, $10.3\%$ of Goalkeepers are misclassified as Player ($6/58$
test instances), driven by the $21\!:\!1$ training imbalance
($12{,}311$ vs $465$ instances) and appearance overlap when
goalkeepers play upfield --- motivating the image-space resolver of
Section~\ref{subsec:methodology_gk_resolver}, evaluated in
Section~\ref{sec:eval_team}.
...
The dedicated ball detector (YOLOv8x, $1280\times1280$, single class,
150 epochs) was trained to repair this failure.
Table~\ref{tab:eval_ball_comparison} compares the two on the ball
class: test \texttt{$\mathrm{mAP}_{50}$} rises from $0.459$ to $0.950$
($+49.1$~pp), and operational recall (IoU=$0.5$, conf=$0.25$) from
$0.69$ to $0.95$. The single-class precision--recall curve
(Figure~\ref{fig:ball_pr}) holds precision $\approx 1.0$ until recall
$\approx 0.95$, in direct contrast to the Ball curve of
Figure~\ref{fig:generic_pr}. This empirically justifies the
dual-detector branch (Section~\ref{sec:methodology_overview}); the
temporal-coherence filter of
Section~\ref{subsec:methodology_ball_detector} then consumes these
detections to produce the per-frame ball trajectory.
...
Metric & Generic det. & Dedicated det. & $\Delta$ \\
\midrule
$\mathrm{mAP}_{50}$         & 0.459 & 0.950 & $+0.491$ \\
$\mathrm{mAP}_{50-95}$       & 0.194 & 0.556 & $+0.362$ \\
Precision       & 0.818 & 0.964 & $+0.146$ \\
Recall (PR)     & 0.393 & 0.896 & $+0.503$ \\
F1              & 0.531 & 0.929 & $+0.398$ \\
\bottomrule
...
\paragraph{Ranking with similar work.}
The generic detector's $0.855$ \texttt{$\mathrm{mAP}_{50}$} sits within the range
reported on broadcast football benchmarks: SoccerNet~\cite{Cioppa2022}
reports mean \texttt{AP} between $0.78$ and $0.91$ across
player-detection submissions, and BroadTrack~\cite{Magera2025} treats
comparable detector quality as a precondition. The dedicated
detector's $0.950$ is competitive with the small-object specialised
detectors surveyed in~\cite{Cioppa2022} --- the relevant regime,
given that the ball typically occupies $<0.05\%$ of the frame. Direct
numerical comparison is hedged by dataset differences.

\paragraph{Verdict on success criterion (i).}
\textbf{Met.} Generic-detector \texttt{$\mathrm{mAP}_{50}$} = $0.855 \geq 0.80$.
The Ball-class shortfall is repaired by the dedicated detector
($0.950$), whose output is the ball signal consumed downstream.
...
% The combined-set HOTA of $66.8$ is close to the SportsMOT results
% reported by Cui~et~al.~\cite{Cui2023SportsMOT}: ByteTrack achieves
% $62.8$--$64.1$ HOTA on the overall SportsMOT test set (depending on
% the Train versus Train+Val protocol), while MixSort-Byte reports
% $66.4$ HOTA on the football category specifically.
...
The tracker is evaluated on a combined $3{,}250$-frame set: five
SportsMOT-football validation sequences~\cite{Cui2023SportsMOT} for
literature comparison, plus one hand-labelled $750$-frame broadcast
clip annotated in MOTChallenge format for in-domain evidence. Metrics
are computed jointly with TrackEval. ByteTrack~\cite{ByteTrack} and
BoT-SORT~\cite{BoTSORT} were evaluated under identical detection input from
the generic YOLOv8m detector (Section~\ref{sec:eval_detection}),
isolating the data-association stage.
...
Tracker & HOTA & MOTA & IDF1 & ID-sw. \\
\midrule
ByteTrack & \textbf{0.668} & 0.883 & \textbf{0.678} & \textbf{2867} \\
BoT-SORT  & 0.657 & \textbf{0.885} & 0.671 & 3929 \\
\bottomrule
\end{tabular}
\caption[Tracker comparison on the combined evaluation
set.]{ByteTrack versus BoT-SORT on a combined evaluation set of
...
ByteTrack achieves higher HOTA ($0.668$ vs $0.657$) and IDF1 ($0.678$ vs
$0.671$) and records approximately $27\%$ fewer identity switches
($2{,}867$ vs $3{,}929$); MOTA differs by $0.2$ points and is within
evaluation noise. ByteTrack is therefore the deployed tracker. Its
edge is consistent with the design intent of two-stage association on
low-confidence detections, which preserves identity through the brief
detection dropouts that broadcast football generates frequently.
BoT-SORT's camera-motion-compensated Kalman filter does not recover
the gap on these sequences. The combined-set HOTA of $0.668$ is close
to the SportsMOT results of Cui~et~al.~\cite{Cui2023SportsMOT}:
ByteTrack reports $0.628$--$0.641$ on the overall test set (Train vs
Train+Val protocol), and MixSort-Byte reports $0.664$ on the football
category specifically.

The dominant residual error in qualitative inspection is identity
switching during close cross-team crossings within a few pixels,
consistent with the failure modes characterised by
BroadTrack~\cite{Magera2025}. The 2{,}867 switches recorded for ByteTrack
across $3{,}250$~frames (Table~\ref{tab:tracker_comparison})
quantify this directly. This is also the upstream cause of
the same-team possession-transition false positives discussed in
Section~\ref{sec:eval_tactical}.

\paragraph{Verdict on success criterion (ii).}
\textbf{Met.} Deployed-tracker HOTA is $0.668$ against a target of
$0.45$, ranks favourably against the published SportsMOT-football
benchmarks, and is corroborated by the in-domain broadcast clip;
residual failures are localised to close-range crossings.
...
% Classical approaches to team identification cluster players by
% RGB colour histograms~\cite{Spagnolo2007}, which are computationally
% cheap but sensitive to lighting variation, shadows, and background
% pixels included in the crop. The SigLIP-based classifier's
% $0.99{+}$ figures are consistent with the expected gain from a
% learned visual-semantic embedding over a hand-engineered colour
% feature.

\section{Team Classification Performance}
\label{sec:eval_team}

The team classifier (Section~\ref{sec:methodology_team}) performs
binary kit-colour separation in SigLIP embedding space, with K-Means
fitted on an early-match warm-up window and then frozen (Section~\ref{subsec:methodology_warmup}). Two
experiments characterise the stage on three independent broadcast
clips, with approximately 300 manually-labelled torso crops sampled
per clip at stride~25.
...
\paragraph{Experiment 2 — 35/65 warm-up split (deployment).}
K-Means is fitted on the first 35\% of crops (temporal order) and the
cluster-team mapping is chosen using those warm-up labels alone. The
remaining 65\% are then classified with the frozen model and scored
against ground truth, mirroring the deployed pipeline, where the classifier sees
only an early portion of the match before predicting the rest.
...
As shown in Table~\ref{tab:team_classifier}, the mean macro-F1 across the three clips is $0.997$ for full-crop
clustering and $0.993$ on the deployment-realistic 35/65 split,
clearing the criterion~(iii) target of $0.85$ comfortably. The gap
between regimes is only $0.4$~pp: a $\sim$100-crop warm-up window is
sufficient for the SigLIP embeddings to generalise to the rest of the
clip, with negligible cost relative to the offline upper bound.
Clip~1's perfect score reflects a visually unambiguous kit pairing
(bright red versus white, Figure~\ref{fig:team_crops_clip1}); clips~2
and~3 use more similar palettes and incur one to three errors apiece,
...
\caption[Example torso crops from clip~1.]{Example torso crops from
clip~1. The bright red versus white kit contrast in clip~1 is visually
unambiguous, which accounts for the perfect macro-F1 of $1.000$ on this clip
in both evaluation regimes. The remaining clips involve more similar palettes
and incur a small number of classification errors.}
\label{fig:team_crops_clip1}
...
\paragraph{Goalkeeper resolver.}
The resolver of Section~\ref{subsec:methodology_gk_resolver} answers a
different question: once the detector has labelled a player as
goalkeeper, to which team do they belong? It assigns the goalkeeper
to the team whose image-space centroid is nearest in the same frame
--- a deterministic rule, not a learned classifier. Therefore, quantitative F1
would require GK-team labels that were not produced; instead, the
resolver was visually verified on the annotated output video for each
test clip and was correct in every spot-checked frame. This evidence
is qualitative and the limitation is acknowledged.

\paragraph{Verdict on success criterion (iii).}
\textbf{Met.} Mean macro-F1 on the held-out 65\% is $0.993$ against a
target of $0.85$. Cross-clip variance is small ($0.984$--$1.000$),
indicating that the result is a property of the embedding space
rather than any one broadcast. Classical approaches cluster players
by RGB colour histograms~\cite{Spagnolo2007}, which are cheap but
sensitive to lighting, shadows, and background pixels in the crop.
The SigLIP-based classifier's $0.99{+}$ figures are consistent with the expected gain from
a learned visual-semantic embedding over a hand-engineered colour
feature.
...
% \paragraph{Ranking with similar work.}
% The mean reprojection error of $1.99$~m sits in the metre-scale
% regime reported for end-to-end broadcast pipelines by
% Theiner~\cite{Theiner2022} (median player-position errors approximately
% $1$--$3$~m) and is broader than the sub-metre calibration accuracy
% of TVCalib~\cite{TVCalib2023} on camera-calibrated SoccerNet sequences.
% Direct numerical comparison is hedged by the different evaluation
% protocols: both works evaluate on calibrated video, whereas this
% work evaluates on keypoint-annotated still frames.

% \paragraph{Verdict on success criterion (iv).}
% \textbf{Met.} Median pitch-space reprojection error is $169.9$\,cm
% (mean $199.4$\,cm), against the criterion threshold of $\leq 250$\,cm.
% The kp\_16 outlier is characterised, mitigated by the confidence-gated
% fit, and ablated quantitatively. Choice of a robust homography estimator
% is identified in Chapter~\ref{ch:conclusions} as the single most impactful
...
broadcast frame coordinates to a $12{,}000 \times 7{,}000$~cm pitch
template (Section~\ref{sec:methodology_pitch}). Their joint accuracy
governs every downstream pitch-space metric --- speeds, distances,
possession, offside lines --- so they are evaluated jointly here.

\paragraph{Keypoint detector.}
The YOLOv8x-pose model achieves test pose \texttt{$\mathrm{mAP}_{50}$} = $0.995$
and \texttt{$\mathrm{mAP}_{50-95}$} = $0.953$ on the 28-image held-out split, with
box detection saturated at $P = R = 1.0$. Per-keypoint pixel errors
(Figure~\ref{fig:per_kp_error}) have a median of $3.7$~px ($0.41\%$
of the $640$~px image diagonal), with one systematic outlier: kp\_16
has mean error $18.2$~px on test ($2.0\%$ of diagonal), corroborated
at $20.3$~px on validation. The outlier is statistically robust
($n = 24$ test, $n = 26$ val) and not a small-sample artefact. Three
further keypoints (kp\_05, kp\_28, kp\_29) have $n < 5$ on test and
are reported with that caveat. The deployed pipeline gates keypoints
by confidence ($\geq 0.5$) and requires at least six before fitting a
homography (Section~\ref{subsec:methodology_homography}); kp\_16 is
therefore already excluded from most fits in practice.
...
\paragraph{Pitch-space reprojection error.}
Pixel error is necessary but not sufficient to evaluate pitch registration: the operational question
is how well the estimated homography $H$ projects ground-truth pixel
positions onto canonical pitch coordinates
(Equation~\ref{eq:reprojection_error}). For each test image with at
least six confident keypoints, $H$ is fitted from predicted pixels to
the canonical vertex set via ordinary least-squares DLT (matching the deployed
\texttt{cv2.findHomography}), then applied to the ground-truth
pixels; the residual against the canonical coordinate is recorded in
centimetres. Table~\ref{tab:reprojection_error} reports aggregate
statistics with and without kp\_16 included in the homography fit and evaluation.
...
The mean residual of $199.4$~cm corresponds to $\approx 1.7\%$ of
pitch length, and the median of $169.9$~cm to $1.4\%$. Excluding
kp\_16 leaves mean and median statistically unchanged
($\Delta < 0.1\%$) but drops the maximum residual from $20.6$~m to $11.5$~m,
confirming that the runtime confidence gate already filters the
outlier from active fits and that the ablation acts only on the upper
tail. The 95th-percentile residual
remains around $5$~m, which is the operational ceiling that
downstream tactical reasoning must be robust to: it is well within
the pitch-third granularity used for possession and field-position
analysis, but is too coarse for sub-metre claims such as
...
A single image (\texttt{2e57b9\_2\_6}) accounts for the
all-32-keypoints maximum of $20.6$~m and a per-image mean of
$5.5$~m, driving most of the gap between median and mean. This is
the regime where a minimal-keypoint, non-robust DLT fit is most
exposed; the deployed pipeline mitigates it through stride caching of
the last-known-good homography
(Section~\ref{subsec:methodology_homography}), discussed further in
Section~\ref{sec:eval_failures}.

\paragraph{Ranking with similar work.}
The mean reprojection error of $1.99$~m sits in the metre-scale
regime reported by Theiner~\cite{Theiner2022} for end-to-end
broadcast pipelines (median player-position errors $\approx 1$--$3$~m)
and is broader than the sub-metre calibration accuracy of
TVCalib~\cite{TVCalib2023} on camera-calibrated SoccerNet sequences.
Direct numerical comparison is hedged by protocol differences: both works evaluate on
calibrated video, whereas this work evaluates on keypoint-annotated
still frames.

\paragraph{Verdict on success criterion (iv).}
\textbf{Met.} Median pitch-space reprojection error is $169.9$\,cm
(mean $199.4$\,cm), against the threshold of $\leq 250$\,cm. The
kp\_16 outlier is characterised, mitigated by the confidence-gated
fit, and ablated quantitatively. Adopting a robust homography
estimator is flagged in Chapter~\ref{ch:conclusions} as the single
most impactful improvement to this stage.
...
% qualitative properties are what success criterion~(vi) of
% Chapter~\ref{ch:introduction} requires.

% \paragraph{Verdict on success criterion (v).}
% \textbf{Met.} Pass detection achieves aggregate $F_1 = 0.903$ across
% three clips, against the threshold of $\geq 0.80$, with a single named
% failure mode (same-team possession transitions during close-range
% overlap) characterised by clip.

% \paragraph{Verdict on success criterion (vi).}
% \textbf{Met.} All ten flagged offside events are positionally correct;
% bird's-eye projections preserve team formation structure across all
% three clips; per-track heatmaps form spatially contiguous regions
% consistent with positional roles. The integrated tactical output
% quality is consistent with the operating regime characterised by the
% failure-handling table of Section~\ref{sec:methodology_failure_handling}.

\section{Tactical Output Quality}
\label{sec:eval_tactical}

This section evaluates the system-level outputs delivered end-to-end:
detected pass and offside events, per-player statistics, and per-track
heatmaps. The three broadcast clips of Section~\ref{sec:eval_team}
were processed through the full pipeline; the resulting JSON event
logs and \texttt{stats.json} were inspected against the annotated
MP4 outputs.

\paragraph{Pass detection.}
Ground truth was obtained by watching each annotated clip and
matching detected passes by passer track identifier and frame index.
Aggregate precision is $0.824$ and recall is $1.000$
(Table~\ref{tab:pass_detection}): every genuine pass is detected, but
six spurious detections occur, all in clip~3. Inspection traced these
to a single failure mode: when two \emph{same-team} players occlude
near the ball, the nearest-ball-holder rule of
Section~\ref{subsec:methodology_passes} briefly flips possession to
the closer player and back, registering a spurious intra-team
\enquote{pass} when the original possessor never lost the ball.
Clips 1 and 2 contained no such overlap and recorded 1.000 precision, 
which is consistent with the deterministic proximity rule having no 
notion of possession beyond the per-frame minimum-distance criterion.
...
\paragraph{Offside detection.}
The pipeline flagged 10 offside events across the three clips
(clip~1: 4, clip~2: 1, clip~3: 5). Each was checked frame-by-frame
against the bird's-eye view, comparing the dashed offside line to the
flagged player's position. All ten were positionally correct: in
every case the flagged player's pitch-space $x$-coordinate lay beyond
the second-last defender in the auto-detected attacking direction
(Section~\ref{subsec:methodology_offside}). This is a
\emph{positional} verdict only --- it does not adjudicate active
interference with play, which the laws-of-the-game definition
requires and which is outside the scope of this work.

\paragraph{Per-player statistics and the speed cap.}
Table~\ref{tab:tactical_stats} summarises per-clip statistics after
filtering fragment tracks (total pitch-space distance below $5$~m, mostly arising from re-detection events at frame edges).
...
Chapter~\ref{ch:methodology}) and the bird's-eye view
(Figure~\ref{fig:pitch_keypoints_and_birdeye}) are qualitative
outputs assessed on visual coherence. Across all three clips the
bird's-eye projection preserves team formation structure (defensive
lines, central midfield clusters), the offside line is rendered
consistently with the offside event log, and per-track heatmaps form
spatially contiguous regions consistent with positional roles.

\paragraph{Verdict on success criterion (v).}
\textbf{Met.} Pass detection achieves aggregate $F_1 = 0.903$ across
three clips against the $\geq 0.80$ threshold, with a single named
failure mode (same-team possession transitions during close-range
overlap) characterised by clip.

\paragraph{Verdict on success criterion (vi).}
\textbf{Met.} The three qualitative properties above hold across all
three clips. The integrated tactical output is consistent with the
operating regime defined by Section~\ref{sec:methodology_failure_handling}.
...
The pipeline was instrumented with per-stage wall-clock timing and
run on a representative $750$-frame clip on Apple~M2~Pro (no discrete
GPU). Table~\ref{tab:runtime_breakdown} reports steady-state per-stage
cost; the team-classifier warm-up
(Section~\ref{subsec:methodology_warmup}) is a one-off $18.2$~s
...
clip.]{Per-stage steady-state runtime for one $750$-frame broadcast
clip on Apple~M2~Pro (no discrete GPU). The warm-up phase
(SigLIP/UMAP/K-Means fit on torso crops) adds a one-off $18.2$~s and
is reported separately. Effective throughput is approximately
$0.55$~fps.}
\label{tab:runtime_breakdown}
\end{table}

Steady-state throughput is $0.55$~fps --- adequate for the offline
analysis use case the pipeline targets. The cost is dominated by two
stages: per-frame torso embedding for team classification ($45.8\%$)
and ball detection at $1280\!\times\!1280$ ($34.6\%$). The generic
detector and tracker together account for under $15\%$, and stride-15
caching reduces pitch registration to under $1\%$, confirming the
caching design as a worthwhile optimisation. Tactical reasoning and heatmap accumulation are negligible. Detector inference is
one to two orders of magnitude slower than the A100 figures ($4.6$~ms generic, $19.8$~ms ball),
reflecting the absence of a CUDA-class accelerator on the M2~Pro
target. Reducing the team-classifier per-frame cost --- by embedding
only on identity-changed tracks, or replacing SigLIP with a lighter
backbone --- is flagged in Chapter~\ref{ch:conclusions} as the single
most impactful runtime optimisation.
...
Ball missed by generic detector
& 31\% bg-confusion (\S\ref{sec:eval_detection})
& Repaired by dedicated detector; ball $\mathrm{mAP}_{50}$ = $0.950$. \\
Goalkeeper-as-Player misclassification
& 10.3\% on test (\S\ref{sec:eval_detection})
& Image-space GK resolver corrects team assignment; verified qualitatively (\S\ref{sec:eval_team}). \\
Homography fit on minimal keypoints
...
The five modes are independent: the failures do not compound
across stages, because each is gated or saturated before its output
reaches the next stage (the dedicated ball detector replaces the generic ball signal; the GK resolver corrects
team labels rather than detector class labels; the speed cap clips
rather than zeroing the underlying signal). The principal
unmitigated mode is the same-team possession-transition pass,
which would benefit from a temporal possession-state machine
...
% The six success criteria committed in
...
% Criterion~(iii), team-assignment macro-$F_1 \geq 0.85$, is comfortably
% met at $0.993$ across three independent clips on the
% deployment-realistic 35/65 evaluation
% (Section~\ref{sec:eval_team}). Criterion~(iv), median pitch-space
% reprojection error $\leq 250$\,cm, is met at $169.9$\,cm
% (mean $199.4$\,cm) on the held-out test split
% (Section~\ref{sec:eval_pitch}). Criterion~(v), pass-detection
% $F_1 \geq 0.80$, is met at $0.903$ across three broadcast clips
% with a single named failure mode
% (Section~\ref{sec:eval_tactical}). Criterion~(vi), qualitatively
% coherent bird's-eye, heatmap, and offside outputs, is met: all ten
% flagged offside events are positionally correct, bird's-eye
% projections preserve team formation structure across all three
% clips, and per-track heatmaps form spatially contiguous regions
% consistent with positional roles (Section~\ref{sec:eval_tactical}).
...
All six success criteria committed in Chapter~\ref{ch:introduction}
are met (Table~\ref{tab:criteria_summary}).
...
\toprule
\# & Criterion & Threshold & Achieved & Section \\
\midrule
i   & Detector $\mathrm{mAP}_{50}$ & $\geq 0.80$ & $0.855$ generic; $0.950$ ball
    & \S\ref{sec:eval_detection} \\
ii  & Tracking HOTA & $\geq 0.45$ & $0.668$ (ByteTrack)
    & \S\ref{sec:eval_tracking} \\
iii & Team macro-$F_1$ (35/65 split) & $\geq 0.85$ & $0.993$ across three clips
    & \S\ref{sec:eval_team} \\
iv  & Median pitch-space error & $\leq 250$~cm & $169.9$~cm (mean $199.4$~cm)
    & \S\ref{sec:eval_pitch} \\
v   & Pass-detection $F_1$ & $\geq 0.80$ & $0.903$ aggregate (3 clips)
    & \S\ref{sec:eval_tactical} \\
vi  & Qualitative output coherence & --- & 10/10 offsides correct;
    bird's-eye and heatmaps coherent across all clips
    & \S\ref{sec:eval_tactical} \\
\bottomrule
\end{tabular}
\caption[Summary of pre-registered success criteria against achieved
results.]{The six pre-registered success criteria against achieved
results. Every criterion is met; the largest margins are on team
classification and pass detection, the tightest on pitch-space
reprojection error.}
\label{tab:criteria_summary}
\end{table}

Three caveats temper these verdicts. First, pitch-space reprojection
error is metre-scale on average (mean $1.99$~m, median $1.70$~m,
Section~\ref{sec:eval_pitch}): this supports tactical-scale reasoning
but not sub-metre geometric claims. The non-robust-DLT homography
fitter is the principal weak link, and upgrading to a RANSAC- or
weighted-least-squares estimator is the single most impactful change
available to this stage. Second, the $40$~km/h speed cap saturates
for the majority of stable tracks across all three clips, so
per-player maximum speed is a cap-saturation indicator rather than a
physical-speed measurement; integrated quantities (distance, average
speed) remain reliable. Third, end-to-end throughput on Apple~M2~Pro
is $0.55$~fps (Section~\ref{sec:eval_runtime}), with $80\%$ of the
budget consumed by team-classifier embedding and ball detection ---
an offline-analysis tool rather than a live system. Each limitation
corresponds to a named architectural choice and is therefore
individually addressable; the prioritisation of remedies is taken up
